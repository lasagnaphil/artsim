...
#include "artsim/utils/pymesh/Exception.h"
...
void MshSaver::save_elements(
        const Eigen::VectorXi& elements, MshSaver::ElementType type) {
    ...
    if (m_num_elements > 0) {
        ...
        if (!m_binary) {
            for (size_t i=0; i<elements.size(); i+=nodes_per_element) {
                ...
                Eigen::VectorXi elem = elements.segment(i, nodes_per_element) +
                    Eigen::VectorXi::Ones(nodes_per_element);
